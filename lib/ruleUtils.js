 ...
 */
export function parseURLforRules(fullURL, hostname, feedsMap) {
  try {
    ...
    // Rule: Stack Exchange Sites
      ...
      if (!origin) {
        console.warn(
          `Skipping Stack Exchange rules for because origin could not be determined.`
        );
        return;
      }
      ...
      if (tagMatch && tagMatch[1]) {
        feedsMap.set(`${origin}/feeds/tag/${tagMatch[1]}`, `${siteName} - [${tagMatch[1]}] Questions`);
      }
      ...
      if (questionMatch && questionMatch[1]) {
        ...
        feedsMap.set(`${origin}/feeds/question/${questionId}`, `${siteName} - Question #${questionId}`);
      }
      ...
      if (userMatch && userMatch[1]) {
        ...
        feedsMap.set(`${origin}/feeds/user/${userId}`, `${siteName} - User Activity`);
      }
    }
  } catch (error) {
