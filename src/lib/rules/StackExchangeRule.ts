...
export class StackExchangeRule implements SiteRule {
  ...
  private extractTagFeed(
    ...
    if (tagMatch?.[1]) {
      feedsMap.set(`${origin}/feeds/tag/${tagMatch[1]}`, {
        title: `${siteName} - [${tagMatch[1]}] Questions`,
        isFromRule: true,
      });
    }
  }
  ...
  private extractQuestionFeed(
    ...
    if (questionMatch?.[1]) {
      ...
      feedsMap.set(`${origin}/feeds/question/${questionId}`, {
        title: `${siteName} - Question #${questionId}`,
        isFromRule: true,
      });
    }
  }
  ...
  private extractUserFeed(
    ...
    if (userMatch?.[1]) {
      ...
      feedsMap.set(`${origin}/feeds/user/${userId}`, {
        title: `${siteName} - User Activity`,
        isFromRule: true,
      });
    }
  }
